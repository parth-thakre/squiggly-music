...
it('connects, browses and retrieves an original stream through a Navidrome-compatible HTTP server', async () => {
  ...
  const server = createServer(async (request, response) => {
    ...
    if (url.pathname === '/music/rest/ping.view') reply({});
    else if (url.pathname === '/music/rest/getAlbumList2.view') {
    ...
    } else if (url.pathname === '/music/rest/getAlbum.view' && params.get('id') === 'a1') {
    ...
    } else if (url.pathname === '/music/rest/stream.view' && params.get('id') === 'song & 1' && params.get('format') === 'raw') {
  ...
  try {
    ...
    const connection = { url: `http://127.0.0.1:${address.port}/music`, username: 'listener', password };
    ...
    expect(requests).toEqual(['ping', 'getAlbumList2', 'getAlbumList2', 'getAlbum', 'stream'].map(endpoint => `/music/rest/${endpoint}.view`));
