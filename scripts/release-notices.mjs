...
// package in the app, then checks that each third-party notice the package must carry
...
function licenseText(dir, name) {
  ...
  // Koffi's per-platform binary packages carry no license file; koffi's own file covers them.
...
export function writeNotices(appDir, platform) {
  ...
  const shippedRoot = join(resources, 'app');
  ...
  for (const [key, meta] of Object.entries(lock)) {
    ...
    if (!isShipped && !RENDERER_ONLY.has(name)) problems.push(`${key} is a production dependency but is not in ${shippedRoot}`);
    ...
    entries.push({ name, version: manifest.version, license, where: isShipped ? `resources/app/${key}` : 'bundled into the renderer (out/renderer)', text });
  }
  ...
  if (platform === 'win32') {
    required.push('resources/runtime/libmpv-2.dll', ...['NOTICE.md', 'mpv-Copyright.txt', 'mpv-LICENSE.GPL.txt', 'mpv-LICENSE.LGPL.txt',
      'ffmpeg-LICENSE.md', 'ffmpeg-COPYING.GPLv3.txt', 'ffmpeg-COPYING.LGPLv3.txt', 'ffmpeg-COPYING.LGPLv2.1.txt'].map(file => `resources/licenses/libmpv-windows/${file}`));
