...
export default function Studies() {
  ...
  const bottomPlayer = <footer className="bottom-player"><div className="mini-track"><Cover index={selected} /><div><strong>{currentTrack.title}</strong><span>{record.artist}</span></div>{heart}</div><div className="bottom-center">{transport}{wave}</div><div className="bottom-tools">{volumeControl}<IconButton label="Open sample queue" onClick={() => setPanel('queue')}><ListMusic size={19} /></IconButton><IconButton label="Inspect audio path" onClick={() => setPanel('signal')}><SlidersHorizontal size={18} /></IconButton></div><IconButton className="compact-signal-trigger" label="Inspect audio path" onClick={() => setPanel('signal')}><SlidersHorizontal size={18} /></IconButton></footer>;
  ...
  function handleLedgerKey(event: KeyboardEvent<HTMLDivElement>) {
    ...
    if (event.key === 'ArrowDown' || event.key === 'ArrowUp') {
      ...
      const activeIndex = ledgerRows.findIndex(row => document.activeElement?.id === `ledger-${row.id}`);
      const step = event.key === 'ArrowDown' ? 1 : -1;
      const next = Math.max(0, Math.min(ledgerRows.length - 1, (activeIndex >= 0 ? activeIndex : focusRow) + step));
      const nextRow = document.getElementById(`ledger-${ledgerRows[next].id}`);
      setFocusRow(next);
    ...
    if (event.key === 'Enter' && event.target === event.currentTarget) {
      event.preventDefault();
      playRow(ledgerRows[Math.min(focusRow, ledgerRows.length - 1)]);
    }
  }
  ...
  return <div className="mock-gallery">
    ...
    <main className={`study study-${study}`} style={study === 'sleeve-notes' ? sleeveStyle : undefined} aria-label={`${direction.name} mockup`}>
      ...
      {study === 'bench' && <>
        ...
        <div className="bench-layout"><aside className="bench-player"><Cover index={selected} /><h2>{currentTrack.title}</h2><p>{record.artist}</p>{wave}{transport}{volumeControl}</aside><section className="bench-main"><h1>Signal path</h1><p>Each stage shows only what the player can read.</p><div className="bench-chain-wrap"><div className="bench-chain">{[
