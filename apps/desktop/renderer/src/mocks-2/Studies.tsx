...
const allTrackRows: TrackRow[] = records.flatMap((record, recordIndex) => [record.track, ...extraTitles[recordIndex]].map((title, albumRow) => ({
  ...
  year: record.year,
  genre: record.genre,
  duration: Math.max(138, record.duration - albumRow * 19 + (recordIndex % 3) * 7),
  albumRow,
})));
...
export default function Studies() {
  ...
  function playRow(row: TrackRow) { setSelected(row.recordIndex); setPosition(0); setPlaying(true); }
  ...
  const lyrics = lyricWords.map((words, index) => ({ at: Math.floor(record.duration * lyricFractions[index]), words }));
  const currentSeconds = record.duration * position / 100;
  ...
  const nextLineAt = lyrics[currentLyric + 1]?.at ?? record.duration;
  ...
  const sleeveRows = [record.track, ...extraTitles[selected]].map((title, rowIndex) => ({ title, duration: Math.max(150, record.duration - rowIndex * 17) }));
  ...
  return <div className="mock-gallery">
    ...
    <main className={`study study-${study}`} style={study === 'sleeve-notes' ? sleeveStyle : undefined} aria-label={`${direction.name} mockup`}>
      ...
      {study === 'bench' && <>
        ...
        <div className="bench-layout"><aside className="bench-player"><Cover index={selected} /><h2>{record.track}</h2><p>{record.artist}</p>{wave}{transport}{volumeControl}</aside><section className="bench-main"><h1>Signal path</h1><p>Each stage shows only what the player can read.</p><div className="bench-chain">{[
      ...
      {study === 'sleeve-notes' && <>
        ...
        <section className="sleeve-lower"><div className="sleeve-tracklist"><div className="sleeve-track-heading"><span>#</span><span>Track</span><span>Length</span></div>{sleeveRows.map(({ title, duration }, rowIndex) => <button key={title} className={trackRow === rowIndex ? 'current' : ''} onClick={() => { setTrackRow(rowIndex); setPosition(0); setPlaying(true); }} aria-pressed={trackRow === rowIndex}><span>{trackRow === rowIndex ? <AudioLines size={16} /> : rowIndex + 1}</span><span><strong>{title}</strong>{trackRow === rowIndex && <span className="inline-wave">{wave}</span>}</span><time>{clock(duration)}</time></button>)}</div><aside className="sleeve-shelf"><h2>More from the shelf</h2>{[1, 2, 3, 4, 5, 6, 7, 0].filter(index => index !== selected).slice(0, 4).map(index => <button key={index} onClick={() => { choose(index); setTrackRow(0); }} aria-label={`Open ${records[index].album}`}><Cover index={index} /><span>{records[index].album}</span></button>)}</aside></section>{bottomPlayer}
      </>}

      {study === 'transistor' && <div className="transistor-desk"><div className="transistor-layout"><figure className="transistor-figure"><div className="mini-window"><div className="mini-top"><Brand /><button className={`top-switch ${keepOnTop ? 'on' : ''}`} role="switch" aria-checked={keepOnTop} onClick={() => setKeepOnTop(value => !value)}><span />Keep on top</button></div><Cover index={selected} /><h1>{record.track}</h1><p>{record.artist}</p>{wave}<div className="mini-transport">{transport}</div><div className="next-up"><h2>Next up</h2>{[1, 2, 3].map(offset => { const index = (selected + offset) % records.length; return <button key={index} onClick={() => choose(index)}><span>{records[index].track}</span><small>{records[index].artist}</small></button>; })}</div><div className="mini-bottom"><IconButton label="Open queue" onClick={() => setPanel('queue')}><ListMusic size={18} /></IconButton><span className="mini-volume">{volumeControl}</span><button className="text-button"><Maximize2 size={15} />Full window</button></div></div><figcaption>Mini player, 380 wide</figcaption></figure><figure className="transistor-figure strip-figure"><div className="strip-window"><Cover index={selected} /><div><strong>{record.track}</strong><span>{record.artist}</span></div><IconButton label={playing ? 'Pause' : 'Play'} onClick={() => setPlaying(value => !value)}>{playing ? <Pause size={15} fill="currentColor" /> : <Play size={15} fill="currentColor" />}</IconButton><IconButton label="Next sample track" onClick={() => choose((selected + 1) % records.length)}><SkipForward size={15} fill="currentColor" /></IconButton><span className="strip-wave">{wave}</span></div><figcaption>Strip, 440 by 64</figcaption><div className="strip-window light"><Cover index={selected} /><div><strong>{record.track}</strong><span>{record.artist}</span></div><IconButton label={playing ? 'Pause' : 'Play'} onClick={() => setPlaying(value => !value)}>{playing ? <Pause size={15} fill="currentColor" /> : <Play size={15} fill="currentColor" />}</IconButton><IconButton label="Next sample track" onClick={() => choose((selected + 1) % records.length)}><SkipForward size={15} fill="currentColor" /></IconButton><span className="strip-wave">{wave}</span></div><figcaption>Strip, light</figcaption></figure></div><p className="transistor-note">Both windows are real sizes, not a scaled desktop.</p></div>}
