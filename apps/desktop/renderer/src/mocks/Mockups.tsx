...
export default function Mockups() {
  ...
  const visibleRecords = records.map((item, index) => ({ ...item, index })).filter(item =>
    ...
    (filter === 'Favorites' ? favorites.includes(item.index) : filter === 'All albums' || filter === 'Tracks' || item.genre === filter));
  function choose(index: number) { setSelected(index); setPosition(0); }
  function changeStudy(value: Study) {
    setStudy(value); setFilter('All albums'); setQuery(''); setPanel(null);
  ...
  const appNav = <nav className="app-nav" aria-label="Collection views">{['All albums', 'Tracks', 'Favorites'].map((item, i) => <button key={item} className={filter === item ? 'selected' : ''} onClick={() => setFilter(item)} aria-pressed={filter === item}>{i === 0 ? <Library size={18} /> : i === 1 ? <Music2 size={18} /> : <Heart size={18} />}{i === 0 ? 'Albums' : item}</button>)}</nav>;
